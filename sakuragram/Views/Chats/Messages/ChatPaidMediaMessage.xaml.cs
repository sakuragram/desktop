...
public partial class ChatPaidMediaMessage : Page
{
    ...
    //                 {
    //                     case TdApi.MessageContent.MessagePaidMedia messagePhoto:
    //                     {
    ...
    public void UpdateMessage(TdApi.Message message)
    {
        ...
        switch (_messageMediaContent)
        {
            case TdApi.MessageContent.MessagePaidMedia messagePaidMedia:
            {
                foreach (var media in messagePaidMedia.Media)
                {
                    switch (media)
                    {
                        case TdApi.PaidMedia.PaidMediaPhoto photo:
                        {
                            _mediaFileId = photo.Photo.Sizes[1].Photo.Id;
                            
                            if (photo.Photo.Sizes[1].Photo.Local.Path != string.Empty)
                            {
                                Image.ImageSource = new BitmapImage(new Uri(photo.Photo.Sizes[1].Photo.Local.Path));
                                SetImageTransform(photo);
                            }
                            else
                            {
                                _client.DownloadFileAsync(fileId: photo.Photo.Sizes[1].Photo.Id, priority: 1);
                            }
                            break;
                        }
                        case TdApi.PaidMedia.PaidMediaPreview paidMediaPreview:
                        {
                            break;
                        }
                        case TdApi.PaidMedia.PaidMediaUnsupported paidMediaUnsupported:
                        {
                            break;
                        }
                        case TdApi.PaidMedia.PaidMediaVideo paidMediaVideo:
                        {
                            break;
                        }
                    }
                ...
                if (messagePaidMedia.Caption.Text != string.Empty)
                {
                    MessageContent.Text = messagePaidMedia.Caption.Text;
    ...
    private void SetImageTransform(TdApi.PaidMedia.PaidMediaPhoto messagePhoto)
    {
